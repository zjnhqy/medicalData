...
export default ({ proxyManager, girder }) => ({
  ...
  mutations: {
    ...
    addToFileList(state, files) {
      for (let i = 0; i < files.length; i++) {
        ...
        // console.log('this is file state', fileState);
        // console.log('this is file length', files.length);
        if (fileInfo.type === 'dicom') {
          fileState.files = fileInfo.list;
          // console.log('dcmstate', fileState);
        }
        if (fileInfo.type === 'remote') {
          ...
          // console.log('this is state', fileState);
        }
  ...
  actions: {
    ...
    promptLocal({ dispatch }, file) {
      const fileList = {
        0: file, // 将文件信息放在索引为 0 的位置
        length: 1, // 指定文件列表的长度为 1
      };
      // console.log('这是filelist', fileList);
      // console.log('这是file', file);
      // console.log(typeof fileList);
      // console.log('调用了一次');
      // console.log('这是列表-数组类型', Array.from(fileList));
      // console.log('这是对象-数组类型', Object.values(fileList));
      dispatch('openFiles', Array.from(fileList)); // 调用 openFiles 函数并传入文件列表
    },
    ...
    openRemoteFiles({ commit, dispatch }, remoteFiles) {
      // console.log('This is remotefile', remoteFiles);
      commit(
        'addToFileList',
    ...
    readFileIndex({ commit, dispatch, state }, fileIndex) {
      ...
      // console.log('state看这个', file.state);
      ...
      if (file.state === 'needsDownload' && file.remoteURL) {
        ...
        // console.log('zjn', file.state, file.remoteURL);
